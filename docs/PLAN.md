...
- **GDPR export/delete** — **built**: `GET /account/export` returns the authenticated user's full data (account info + every domain row they own) as a single JSON response, synchronous — no file storage or email-attachment infra needed given this is bounded per-user data, not a huge export. **Revisit with an async (generate a file, email a link) approach if a real user's data ever gets large enough that this becomes slow or memory-heavy** — flagged explicitly, not a decision made now. `DELETE /account` (body `{ confirm: true }`, a deliberate extra guard beyond just a valid access token given this is the single most destructive action in the app) hard-deletes everything for that user in one transaction. Neither of these could rely on cascading deletes: none of `BudgetMonth`/`Category`/`CategoryMonth`/`Transaction`/`RecurringExpense`/`SavingsFund`/`SavingsMovement` has an actual FK relation back to `User` in the schema (`userId` is an application-scoped column only) — only `RefreshToken` does. So deletion is an explicit, dependency-ordered multi-table wipe (children before parents, respecting the `Restrict` FKs between domain tables), plus a separate `otp_codes` cleanup by email (keyed by email, not `userId`, so outside the normal per-user chain). Verified against real Postgres, not just the fake, given how many `Restrict` constraints the ordering has to walk through correctly. A privacy policy is still not written — a product/legal task, not a code one. This is REST, not GraphQL, same reasoning as `/auth/*` and `logout-all` — see `docs/PROGRESS.md`.

> **Known limitation, queued as the next task, not fixed here**: the missing `User` FK relations above aren't a considered tradeoff — this Data Model section has always labeled every one of these `user_id` columns `(fk)`, but only `refresh_tokens` actually got a real database-level constraint (`REFERENCES "users"("id")`, confirmed in the init migration SQL); the rest never did, silently, since Prisma only generates a real FK when a named `@relation` is declared, and app-level `WHERE userId = ...` filtering already made every query behave correctly without one. This surfaced now because `deleteAccount` is the first thing that ever needed to delete *by* `userId` across these tables. The practical consequence: a row created for this `userId` in `Category`/`BudgetMonth`/`SavingsFund` (the three with no other FK dependency) at the exact moment `deleteAccount` runs — or immediately after — can become a permanently orphaned row, since nothing links it back to a `User` that still exists to ever delete it later. Retrofitting real FKs isn't a small fix: making them `onDelete: Cascade` from `User` would need to cascade straight through the *intentional* `Restrict` relations already sitting between the domain tables themselves (e.g. `CategoryMonth → Category`, deliberately `Restrict` so a normal single-item `deleteCategory` can't silently wipe out linked data) — weakening a real product safety guard, not just implementation plumbing. Scoped as its own grilled, designed piece of work next, not folded into the GDPR branch.
