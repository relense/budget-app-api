...
on `postinstall` alongside the Prisma client) — a schema change unmatched
by a resolver is a compile error, not a silent drift. Nested-type
resolvers (`Category`, `CategoryMonth`, etc.) still use their own narrow
per-field inline typing, not the generated per-type `Resolvers` — see
`PLAN.md`'s "Schema-to-types safety" note for why (and the residual gap
that leaves).
