...
>
> `Category`, `CategoryMonth`, `Transaction` reflect finalized, grilled designs from step 3 — trust them now. `RecurringExpense` reflects the flat redesign that supersedes step 4's original `RecurringExpenseTemplate`/`RecurringExpenseInstance` split — trust the shape, but it hasn't been implemented yet (see PROGRESS.md).
